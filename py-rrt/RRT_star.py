# Author: Adwait Naik @addy1997 | Created on: 12/01/2019
#!/usr/bin/python
# -*- coding: utf-8 -*-
...
def init_obstacles(configNum):  # initialized the obstacle
    global rectObs
    rectObs = []
    rectObs.append(pygame.Rect((XDIM / 2.0 + 200, YDIM / 2.0 - 180),
                   (100, 370)))
        ...
        if currentState == 'init':
            ...
            if count < NUMNODES:
                ...
                while foundNext == False:
                    ...
                    parentNode = nodes[0]
                    for p in nodes:
                        if dist(p.point, rand) \
                            <= dist(parentNode.point, rand) \
                            and dist(p.point, rand2) \
                            <= dist(parentNode.point, rand2) \
                            and dist(p.point, rand3) \
                            <= dist(parentNode.point, rand3) \
                            and dist(p.point, rand4) \
                            <= dist(parentNode.point, rand4) \
                            and dist(p.point, rand5) \
                            <= dist(parentNode.point, rand5) \
                            and dist(p.point, rand6) \
                            <= dist(parentNode.point, rand6) \
                            and dist(p.point, rand7) \
                            <= dist(parentNode.point, rand7) \
                            and dist(p.point, rand8) \
                            <= dist(parentNode.point, rand8) \
                            and dist(p.point, rand9) \
                            <= dist(parentNode.point, rand9) \
                            and dist(p.point, rand10) \
                            <= dist(parentNode.point, rand10):
                            ...
                            if collides(newPoint) == False \
                                and collides(newPoint2) == False \
                                and collides(newPoint3) == False \
                                and collides(newPoint4) == False \
                                and collides(newPoint5) == False \
                                and collides(newPoint6) == False \
                                and collides(newPoint7) == False \
                                and collides(newPoint8) == False \
                                and collides(newPoint9) == False \
                                and collides(newPoint10) == False:
                                parentNode = p
                                foundNext = True
                ...
                pygame.draw.line(screen, blue, parentNode.point,
                                 newnode)
                pygame.draw.line(screen, blue, parentNode.point,
                                 newnode2)
                pygame.draw.line(screen, blue, parentNode.point,
                ...
                if point_circle_collision(newnode, goalPoint.point,
                        GOAL_RADIUS) \
                    or point_circle_collision(newnode2,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode3,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode4,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode5,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode6,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode7,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode8,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode9,
                        goalPoint.point, GOAL_RADIUS) \
                    or point_circle_collision(newnode10,
                        goalPoint.point, GOAL_RADIUS):
